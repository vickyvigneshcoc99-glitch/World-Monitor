...
fn open_live_channels_window(app: &AppHandle, base_url: Option<String>) -> Result<(), String> {
    ...
    .title("Channel management - RetailSaathi")
...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        name: Some("RetailSaathi".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        copyright: Some("\u{00a9} 2025 vickyvigneshcoc99-glitch".into()),
        website: Some("https://github.com/vickyvigneshcoc99-glitch/World-Monitor".into()),
        website_label: Some("github.com/vickyvigneshcoc99-glitch/World-Monitor".into()),
        ..Default::default()
    };
    let about_item =
        PredefinedMenuItem::about(handle, Some("About RetailSaathi"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/vickyvigneshcoc99-glitch/World-Monitor");
        }
        #[cfg(feature = "devtools")]
        MENU_HELP_DEVTOOLS_ID => {
